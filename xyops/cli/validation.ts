...
const requireSuccessfulEnvelope = <T>(value: import("./types").VoiceflowEnvelope<T>, operation: string): T => {
  if (value.ok === false) throw fail("envelope", { nextAction: `${operation} was rejected by the migration runner.` });
  return value.result;
};
