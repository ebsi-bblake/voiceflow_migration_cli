...
const readStringChunk = (chunk: unknown): string | undefined => typeof chunk === "string" ? chunk : undefined;
const readBytesChunk = (chunk: unknown, decoder: TextDecoder): string | undefined => chunk instanceof Uint8Array ? decoder.decode(chunk, { stream: true }) : undefined;
const firstDefined = (values: readonly (string | undefined)[]): string => values.find((value) => value !== undefined) ?? "";
const readChunk: ReadChunk = (chunk, decoder) => firstDefined([readStringChunk(chunk), readBytesChunk(chunk, decoder)]);
...
const newlineEnd = (buffer: string, index: number): number => index + (Number(buffer.startsWith("\r\n", index)) + 1);
...
const stringifyFallback = (value: unknown): string => JSON.stringify(value) ?? String(value);
const stringifyValue = (value: unknown): string => typeof value === "string" ? value : stringifyFallback(value);
const sanitizeText = (text: string): string => [...text].map((character) => isControlCharacter(character) ? " " : character).join("");
const truncateText = (text: string, max: number): string => text.length > max ? `${text.slice(0, max)}…` : text;
export const bounded: Bounded = (value, max = 200) => truncateText(sanitizeText(stringifyValue(value)), max);

type ChooseOption = (reader: PromptReader, title: string, options: readonly Option[]) => Promise<string>;
const isOptionNumber = (number: number, length: number): boolean => number >= 1 && number <= length;
const requireOptions = (title: string, options: readonly Option[]): readonly Option[] => {
  if (!options.length) throw new Error(`No ${title.toLowerCase()} options were returned.`);
  return options;
};
const readOption = (reader: PromptReader, options: readonly Option[]): Promise<string> => reader.ask("Select number: ").then((answer) => {
  const number = Number.parseInt(answer, 10);
  if (isOptionNumber(number, options.length)) return options[number - 1].value;
