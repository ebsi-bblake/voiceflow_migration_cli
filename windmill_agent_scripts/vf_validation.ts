...
const parseString = (value: unknown, maximum: number): string => {
  ...
  if (!normalized || Array.from(normalized).length > maximum || hasControl || VoiceflowRegex.pathSeparator.test(normalized))
...
export const parseFolderID = (value: unknown): string => {
  ...
  if (!VoiceflowRegex.numericID.test(id)) throw new OperationFault("INVALID_ARGUMENT");
  return id;
};
export const parseSchemaVersion = (value: unknown): string => {
  if (typeof value !== "string") throw new OperationFault("INVALID_ARGUMENT");
  const schema = value.trim();
  if (!schema || Array.from(schema).length > 40 || VoiceflowRegex.controlCharacter.test(schema))
    throw new OperationFault("INVALID_ARGUMENT");
  return schema;
};
