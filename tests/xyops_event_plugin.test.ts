...
describe("native XYOps event plugin boundary", () => {
  ...
  test("maps Voiceflow success and failure envelopes to protocol responses", () => {
    ...
    expect(failureResponse).toMatchObject({ xy: 1, complete: true, code: "AUTHENTICATION_FAILED", description: "[pluginVersion=0.1.0] Authentication failed. (code=AUTHENTICATION_FAILED)" });
  ...
  test("reports the response stage and redacts sensitive error details", async () => {
    ...
    expect(diagnostics).toMatch(/^pluginVersion=0.1.0 stage=response error=Error message=/);
  ...
  test("formats a bounded diagnostic with an input stage", () => {
    ...
    expect(diagnostic).toMatch(/^pluginVersion=0.1.0 stage=input error=Error message=/);
  ...
  test("runs a CJS bundle from an extensionless path with Node", () => {
    ...
    try {
      ...
      expect(stderr).toBe(
        "pluginVersion=0.1.0 stage=secret error=PluginValidationFault message=The Voiceflow JWT secret is not configured.",
      );
    } finally {
